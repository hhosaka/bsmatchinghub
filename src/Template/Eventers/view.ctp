...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        -->
        <li><?= $isadmin && $exists ? $this->Html->link(__('Delete'), [ 'action' => 'delete_queue_self', $eventer->id]) : ''?> </li>
...
<div class="eventers view large-9 medium-8 columns content">
    ...
    <div class="related">
        ...
        <table cellpadding="0" cellspacing="0">
            <tr>
                <th scope="col"><?= __('Challenger') ?></th>
                <th scope="col" width ="15%"><?= __('Status') ?></th>
                <th scope="col" width ="15%"><?= __('Date') ?></th>
                <?php if($isadmin):?><th scope="col" width ="10%" class="actions"><?= __('Actions') ?></th><?php endif; ?>
            </tr>
            ...
            <tr>
                ...
                <td><?= h(date('H:i',strtotime($queues->creation_date))) ?></td>
                <?php if($isadmin):?>
                <td class="actions">
                    <?= $isadmin ? $this->Form->postLink(__('Del'), ['action' => 'delete_queue', $eventer->id, $queues->id], ['confirm' => __('Are you sure you want to delete # {0}?', $queues->user->handlename)]) : '***' ?>
                </td>
                <?php endif; ?>
            </tr>
            <?php endforeach; ?>
